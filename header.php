					...
					<?php if ( isset( $search_enabled ) && 1 === $search_enabled ) : ?>
						<form class="navbar-form navbar-right" role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
							<div class="input-group input-group-sm">
								<input type="text" id="s" name="s" class="form-control" placeholder="<?php _e( 'Search', 'bootstrap-ag' ); ?>">
								<span class="input-group-btn">
									<button type="submit" id="searchsubmit" name="submit" class="btn btn-default"><?php _e( 'Search', 'bootstrap-ag' ); ?></button>
							</span>
							</div>
						</form>
					<?php endif; ?>
