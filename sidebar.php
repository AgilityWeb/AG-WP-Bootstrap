	...
	<?php if ( is_active_sidebar( 'primary_widget_area' ) ) : ?>
		...
		<div id="widget-area" class="widget-area" role="complementary">
			...
			<?php if ( current_user_can( 'manage_options' ) ) : ?>
				<p class="edit-link"><a href="<?php echo admin_url( 'widgets.php' ); ?>" class="badge badge-info"><?php _e( 'Edit', 'bootstrap-ag' ); ?></a></p><!-- Show Edit Widget link -->
			<?php endif; ?>
		</div><!-- .widget-area -->
	...
	<?php if ( is_archive() || is_single() ) : ?>
		...
		<div class="well sidebar-nav">
			<div id="primary-two" class="widget-area">
				<?php
					$output = '<ul class="recentposts">';
						...
						if ( $recentposts_query->have_posts() ) :
							$output .= '<li><h3>' . __( 'Recent Posts', 'bootstrap-ag' ) . '</h3></li>';
							while ( $recentposts_query->have_posts() ) : $recentposts_query->the_post();
								$output .= '<li>';
									...
									$month_check = $month;
								$output .= '<h4><a href="' . get_the_permalink() . '" title="' . sprintf( __('Permalink to %s', 'bootstrap-ag'), the_title_attribute('echo=0') ) . '" rel="bookmark">' . get_the_title() . '</a></h4>';
								$output .= '</li>';
							endwhile;
						endif;
				...
				<br />
				<ul>
					<li><h3 class="border-bottom"><?php _e('Categories', 'bootstrap-ag'); ?></h3></li>
					<?php
					...
					<?php if ( ! is_author() ) : ?>
						<li>&nbsp;</li>
						<li><a href="<?php echo get_permalink( get_option( 'page_for_posts' ) ); ?>" class="btn btn-default"><?php _e('more', 'bootstrap-ag'); ?></a></li>
					<?php endif; ?>
				</ul>
			</div><!-- /#primary-two -->
		</div>
